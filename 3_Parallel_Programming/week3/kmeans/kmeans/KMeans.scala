...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val defaultMap    : GenMap[Point, GenSeq[Point]] = GenMap() ++ (means map {(_ -> GenSeq())})
    val classifiedMap : GenMap[Point, GenSeq[Point]] = points groupBy {findClosest(_, means)}

    defaultMap ++ classifiedMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    lazy val meanPair = oldMeans zip newMeans
    meanPair forall
      { case (o, n) =>
        o.squareDistance(n) <= eta}
  }
